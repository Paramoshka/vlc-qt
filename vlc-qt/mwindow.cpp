...
void Mwindow::initUI() {
    ...
    //my btn
    QAction *playURL = new QAction("&PlayUrl", this);
    ...
    //my slot
    connect(playURL, SIGNAL(triggered()), this, SLOT(playUrl()));
...
void Mwindow::playUrl() {

    /* The basic file-select box */
    // QString url = "http://web.landoffreedom.ru:8088/video/2.ISO";
    QString path = "/home/ivan/iso/2.ISO";
    ...
    /* Create a new Media */
    //libvlc_media_t *vlcMedia = libvlc_media_new_location(vlcInstance, qtu(url));
    libvlc_media_t *vlcMedia = libvlc_media_new_path(vlcInstance, qtu(path));
    if (!vlcMedia)
        return;
    ...
    libvlc_media_player_set_nsobject(vlcPlayer, (void *)videoWidget->winId());
#elif defined(Q_OS_UNIX)
    videoWidget->attachVlcPLayer(vlcPlayer);
    libvlc_media_player_set_xwindow(vlcPlayer, videoWidget->winId());
#elif defined(Q_OS_WIN)
    libvlc_media_player_set_hwnd(vlcPlayer, (HWND)videoWidget->winId());
#endif
...
void Mwindow::keyPressEvent( QKeyEvent* event ) {
    if(vlcInstance) {
        switch ( event->key() ) {
        case Qt::Key_W:
            libvlc_media_player_navigate(vlcPlayer, 1);
            break;
        case Qt::Key_S:
            libvlc_media_player_navigate(vlcPlayer, 2);
            break;
        case Qt::Key_A:
            libvlc_media_player_navigate(vlcPlayer, 3);
            break;
        case Qt::Key_D:
            libvlc_media_player_navigate(vlcPlayer, 4);
            break;
        case Qt::Key_F:
            libvlc_media_player_previous_chapter(vlcPlayer);
            break;
        default:
            event->ignore();
            break;
